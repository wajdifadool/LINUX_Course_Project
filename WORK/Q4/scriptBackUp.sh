...
if [[ "$#" -eq 1 ]]; then
  ...
  if [[ -f "$1" && "$1" == *.csv ]]; then
    ...
    echo "Error: '$1' not valid .csv file " 
...
do
    ...
    # Ensure plant_name is not empty
    # if [[ -n "$plant_name" ]]; then
    #     echo "Processing plant: $plant_name" >> $LOG_FILE
    #     echo "Processing plant: $plant_name" 
        
    #     # Create a directory for the plant inside 'Diagrams'
    #     mkdir -p "Diagrams/$plant_name"
        
    #     # Run the Python script with the parameters from the CSV file
